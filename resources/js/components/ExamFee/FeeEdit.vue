                  ...
                  <div class="row">
                    ...
                    <div class="col-md-5">
                      <div class="input-group mb-3">
                        ...
                        <select class="form-control" @change="examEvent" v-model="form.exam_type_id">
              ...
              <div class="card">
                ...
                <div class="card-body">
                  ...
                  <div class="text-center">
                    <button type="submit" class="btn btn-primary mt-2" disabled v-if="form.year_id.length === 0 || form.exam_type_id.length === 0 || form.amount.length === 0 || form.class_id.length === 0">Update Listed</button>
...
export default {
  ...
  methods:{
    ...
    updateData(){
      ...
      axios.put('/examfee/'+this.$route.params.year+'/'+this.$route.params.exam, this.form)
          .then(response => {
    ...
    getByIdData(){
      axios.get('/examfee/'+this.$route.params.year+'/'+this.$route.params.exam)
          .then(response => {

            this.form.exam_type_id = response.data[0].exam_type_id
            this.form.year_id = response.data[0].year_id;
            response.data.forEach(exam => {
              this.show_exam = exam.exam.name;
              this.year_show = exam.year.name;
              this.form.amount.push(exam.amount);
              this.form.class_id.push(exam.student_class.id);
              this.class_show.push(exam.student_class.name);
            })
          })
    },
    validateData(){
